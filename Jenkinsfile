            ...
            steps {
                sh '''
                    docker run \
                        ...
                        aquasec/trivy image mydevopsuser46/devops-mega-project:latest \
            ...
            steps {
                sh """
                    curl -v -k --user admin:${JENKINS_API_TOKEN} \
                        ...
                        'http://13.126.66.110:8080/job/Gitops-devops-mega-project/buildWithParameters?token=gitops-token'
                """
            }
        }
    }
